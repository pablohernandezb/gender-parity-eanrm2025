...
def main():
    ...
    # If party_positions is a dict with a 'data' key, use that
    ...
    # Get all unique code_organization values for parties
    party_codes = set()
    for party in party_positions:
        if isinstance(party, dict) and party.get("code_organization"):
            party_codes.add(party["code_organization"])
    ...
    for circ in circunscriptions:
        ...
        for party in relevant_parties:
            ...
            print(f"Fetching candidates for circunscription {circ_name} ({circ_id}), party {code_organization}")
